...
| NexusPiercerPatterns | JSON quality report and structure profiling over a PATH, not a `Dataset` (NOT ETL recipes — `jsonToDelta` and friends do not exist; OSS-01) | src/main/java/.../spark/NexusPiercerPatterns.java |
...
2. **How does AvroReconstructor work with flattened data?**
   - **Answer (corrected 2026-08-19):** AvroReconstructor (3,922 lines, measured) uses the Avro schema to rebuild hierarchical records from flattened Maps. **It does NOT confirm "perfect reconstruction" and the phrase should not be used.** `verify()` compares doubles with an absolute tolerance of 1e-6, and - the mechanism that actually loses money - `compatibleTypes` admits any `Number` beside any `Number`, after which `compareNumbers` falls through to `a.longValue() == b.longValue()` for every non-floating type. Two `BigInteger`s that agree only in their low 64 bits therefore verify as identical: measured, `18446744073709551621` and `129127208515966861317` both have `longValue()` 5 and the verdict is PERFECT. **A previous version of this line said `verify()` treats String and Number as a compatible pair. Measured, it does not** - `compatibleTypes` has arms for same-class, Number/Number, Map/Map and List/List and then returns false, so a String beside a Number is reported as a type mismatch. Use `Map.equals`. Two further limits are published in `SECURITY.md`: `reconstruct()` returns a datum that fails `GenericData.validate` whenever the record has nesting (`recon/NP-022`), and a defaulted logical-type field arrives as its raw underlying type (`recon/NP-026`).
