...
class AvroFlattenReconstructStressTest {
    ...
    class StressTests {
        ...
        @Test
        @DisplayName("Wide record (50 fields)")
        void testWideRecord() {
            def builder = SchemaBuilder.record("WideRecord").fields()

            (1..50).each { i ->
                builder = builder.requiredString("field${i}")
            }
            ...
            verifyRoundTrip(data, schema, "Wide Record (50 fields)")
        }

        @Test
        @DisplayName("Complex real-world-like schema")
        void testRealWorldSchema() {
            // Address schema
            Schema addressSchema = SchemaBuilder.record("Address")
                    .fields()
                    .requiredString("line1")
                    .optionalString("line2")
                    .requiredString("city")
            ...
            Schema personSchema = SchemaBuilder.record("Person")
                    ...
                    .name("workAddress").type(
                    Schema.createUnion(Schema.create(Schema.Type.NULL), addressSchema)
            ).noDefault()
                    .name("contacts").type().array().items(contactSchema).noDefault()
                    .endRecord()
            ...
            // Main order schema
            Schema schema = SchemaBuilder.record("CompleteOrder")
                    .fields()
                    .requiredString("orderId")
                    .requiredLong("orderTimestamp")
                    .requiredString("status")
                    .name("customer").type(personSchema).noDefault()
                    ...
                    .optionalString("specialInstructions")
                    .name("tags").type().array().items().stringType().noDefault()
                    .endRecord()
            ...
            Map<String, Object> data = [
                    ...
                    customer: [
                            ...
                            contacts: [
                                    ...
                                    [type: "PHONE", value: "+1-555-123-4567", isPrimary: false],
                                    [type: "EMAIL", value: "jdoe@work.com", isPrimary: false]
                            ]
                    ],
                    items: [
                            [
                                    ...
                                    unitPrice: 49.99,
                                    totalPrice: 99.98,
                                    notes: ["Gift wrap requested", "Include receipt"]
                            ],
                            [
                                    productId: "PROD-002",
                            ...
                            [
                                    ...
                                    unitPrice: 19.99,
                                    totalPrice: 59.97,
                                    notes: ["Fragile", "Handle with care", "Do not bend"]
                            ]
                    ],
                    subtotal: 289.94,
                    tax: 23.20,
                    ...
                    specialInstructions: "Please leave at front door if no one answers",
                    tags: ["priority", "verified-customer", "repeat-buyer"]
            ]

            verifyRoundTrip(data, schema, "Real World Order Schema")
        }

        @Test
        @DisplayName("Performance test - many iterations")
        void testPerformance() {
            ...
            Schema schema = SchemaBuilder.record("Container")
                    .fields()
                    .requiredString("containerId")
                    .name("items").type().array().items(itemSchema).noDefault()
                    .endRecord()

            Map<String, Object> data = [
                    containerId: "CONT-001",
                    items: (1..20).collect { i ->
                        [id: "ITEM-${i}", name: "Item ${i}", value: i * 100]
                    }
            ]
            ...
            iterations.times { i ->
                Map<String, Object> flattened = flattener.flatten(data)
                Map<String, Object> reconstructed = reconstructor.reconstructToMap(flattened, schema)
            }
            ...
            println "Performance Test: ${iterations} iterations in ${totalMs}ms (avg: ${avgMs}ms per iteration)"

            // Verify correctness on last iteration
            verifyRoundTrip(data, schema, "Performance Test Final")
        }
    }
}